...
All figures have 
- **state** - all figures have `color`, but each baseFigure type can also have one or several unique properties (`radius` for circle, `firstLeg` and `secondLeg` for right triangle, and so on).
- **behavior** - we can obtain the area of any baseFigure and are able to draw it. To 'draw' means to print out all information about a baseFigure using `System.out.println()` (you shouldn't override the toString() method for this). 
